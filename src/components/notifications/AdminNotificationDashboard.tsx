...
const AdminNotificationDashboard = () => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        ...
        const response = await fetch('/api/notifications/admin?' + urlSearchParams);
        ...
        setError((err as Error).message || 'Failed to load admin notifications');
      } finally {
        setLoading(false);
      }
    };
    fetchNotifications();
  }, [currentPage, startDate, endDate, selectedType, limit]);
  ...
  const exportNotifications = () => {
    ...
    const csvContent = notifications
      .map((item) => {
        return `"${item._id}","${item.recipient?.name || 'N/A'}","${item.title || ''}","${item.message.replace(
          /"/g,
          '""'
    ...
    <div className="px-6 py-6 max-w-full">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">View and manage all notifications in the system</p>
        </div>
              ...
              <tbody className="divide-y divide-gray-200">
                {notifications.map((item) => (
                  <tr key={item._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getTypeStyles(
                          item.type
                        )}`}
                      >
                        {item.type}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {item.timeAgo || format(new Date(item.createdAt), 'MMM d,㴼 h:mm a')}
                    </td>
                  </tr>
                ))}
              </tbody>
